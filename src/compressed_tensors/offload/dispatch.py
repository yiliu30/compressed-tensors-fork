...
def get_device_memory() -> dict[torch.device, int]:
    """
    Get the total memory of all available devices. Returns accelerator device memory
    ...
    if dist.is_available() and dist.is_initialized():
        ...
        free, total = torch.accelerator.get_memory_info(dist.get_rank())
        return {torch.device(accel_type): total}
