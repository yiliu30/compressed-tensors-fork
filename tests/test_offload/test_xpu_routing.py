...
def _emulate_xpu_active(config) -> bool:
    """Check if --emulate-xpu is active (is_accelerator_type is globally patched)."""
    return getattr(config, "_emulate_device_remap_mode", None) is not None
